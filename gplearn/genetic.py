...
from wrappers.circuscharlie import SimpleCircusCharlieWrapper
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, env: gym.Env, seeds, params):
    """Private function used to build a batch of programs within a job."""
    if isinstance(env, str):
        try:
            env = make_atari_env(env, n_envs=4)
            env = VecFrameStack(env, n_stack=4)
        except:
            if "Charlie" in env:
                env = SimpleCircusCharlieWrapper(retro.make(env))
            elif "Seaquest" in env:
                env = SimpleSeaquestWrapper(retro.make(env))
            else:
                raise NotImplementedError
    if X is not None:
        n_samples, n_features = X.shape
    else:
        n_samples, n_features = 45, env.observation_space.shape[0]  # n_samples chosen arbitrarily
    # Unpack parameters
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            # import torch.optim
            # parent.to_torch()
            # optim = torch.optim.SGD(parent.parameters(), 0.01)
            # while True:
            #     optim.zero_grad()
            #     out = parent(np.array([[1, 1, 1, 1]]))
            #     loss = (0 - out) ** 2
            #     loss.backward()
            #     optim.step()
            #     print(loss, out)
            ...
            # import graphviz
            #
            # make_dot(out, params=dict(list(parent.named_parameters()))).render("rnn_torchviz", format="png")
            # graphviz.Source(parent.export_graphviz()).render("rnn_graphviz", format="png")
            ...
            if method < method_probs[0]:
                ...
                program, mutated = parent.constants_sgd(env)
                genome = {"method": "Constants SGD", "parent_idx": parent_index, "parent_nodes": mutated}
            else:
                # reproduction
        ...
        program.raw_fitness_ = program.raw_fitness(X, y, curr_sample_weight, env)
        if max_samples < n_samples:
            # Calculate OOB fitness
            program.oob_fitness_ = program.raw_fitness(X, y, oob_sample_weight, env)
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.p_constants_sgd = p_constants_sgd
    ...
    def fit(self, X=None, y=None, sample_weight=None, env: gym.Env = None):
        ...
        """
        assert (X is not None and y is not None) or (env is not None)
        ...
        if X is not None:
            ...
            self.n_features_ = env.observation_space.shape
        ...
        if self.feature_names is not None:
            if self.n_features_ != len(self.feature_names):
                raise ValueError(
                    ...
                    "length to n_features. Expected %d, got %d." % (self.n_features_, len(self.feature_names))
                )
        ...
        if isinstance(env, stable_baselines3.common.vec_env.vec_frame_stack.VecFrameStack):
            env = env.unwrapped.envs[0].unwrapped.spec.id
        elif isinstance(env, (SimpleCircusCharlieWrapper, SimpleSeaquestWrapper)):
            env = env.unwrapped.gamename
        for gen in range(prior_generations, self.generations):
            ...
            population = Parallel(n_jobs=n_jobs, verbose=int(self.verbose > 1))(
                delayed(_parallel_evolve)(
                    n_programs[i], parents, X, y, sample_weight, env, seeds[starts[i] : starts[i + 1]], params
                )
...
class SymbolicRegressor(BaseSymbolic, RegressorMixin):
    ...
    def __init__(
        ...
        super(SymbolicRegressor, self).__init__(
            ...
            p_point_replace=p_point_replace,
            p_constants_sgd=p_constants_sgd,
    ...
    def predict(self, X):
        ...
        if self.n_features_ != n_features:
            raise ValueError(
                ...
                "n_features is %s." % (self.n_features_, n_features)
            )
