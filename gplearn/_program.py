...
from .rl import CustomActorCriticPolicy
from .utils import check_random_state


class _Program(nn.Module):
    ...
    @staticmethod
    def _apply_torch_function(function_numpy, *operands):
        return _function_map[function_numpy.name + "_torch"](*operands)

    def forward(self, X):
        if not isinstance(X, torch.Tensor):
            X_th = torch.tensor(X, requires_grad=False).float()
        else:
            X_th = X
        node = self.program[0]
        if isinstance(node, float):
            return torch.tensor(node).repeat_interleave(X_th.shape[0])
        if isinstance(node, int):
            return X_th[:, node]
        ...
        for node in self.program:
            ...
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:
                ...
                for t in apply_stack[-1][1:]:
                    if isinstance(t, int):
                        terminals.append(X_th[:, t])
                    elif isinstance(t, (nn.Parameter, torch.Tensor)):
                        # if len(t.shape) == 0:
                        if len(t.shape) and t.shape[0] == X_th.shape[0]:
                            terminals.append(t)
                        else:
                            terminals.append(torch.repeat_interleave(t, X_th.shape[0]))
                        # else:
                        #     assert X_th.shape[0] == t.shape[0]
                    else:
                        raise Exception("Somethings wrong!")
                # terminals = [
                # torch.repeat_interleave(torch.tensor(t, requires_grad=True), X_th.shape[0])
                # if isinstance(t, float)
                # else t.clone().detach().requires_grad_(True).repeat_interleave(X_th.shape[0])
                # if isinstance(t, torch.FloatTensor)
                # else X_th[:, t]
                # if isinstance(t, int)
                # else torch.tensor(t, requires_grad=True)
                # for t in apply_stack[-1][1:]
                # ]
                intermediate_result = self._apply_torch_function(function, *terminals)
                if len(apply_stack) != 1:
                    apply_stack.pop()
                    apply_stack[-1].append(intermediate_result)
                else:
                    assert isinstance(intermediate_result, torch.Tensor)
                    return intermediate_result.unsqueeze(1)
    ...
    def raw_fitness(self, X, y, sample_weight, env):
        ...
        if X is not None:
            ...
            raw_fitness = self.metric(self, env)
    ...
    def constants_sgd(self, env):
        program = deepcopy(self)
        mutate = []
        ...
        if len(mutate) > 0:
            ...
            get_rewards(program, env, render=False, assert_torch=False)
            program.to_torch()
            model = PPO(CustomActorCriticPolicy, env, policy_kwargs={"program": program}, device="cpu", verbose=1)
            model.learn(5_000)
            program.to_numpy()
            get_rewards(program, env, render=False, assert_torch=False)
        return program.program, mutate

    def to_torch(self):
        for i, each in enumerate(self.program):
            if isinstance(each, float):
                self.program[i] = nn.Parameter(torch.tensor(each))
                exec(f"self.parameter_{i} = self.program[i]")

    def to_numpy(self):
        for i, each in enumerate(self.program):
            if isinstance(each, nn.Parameter):
                self.program[i] = each.item()
