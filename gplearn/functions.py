...
def _protected_division_torch(x1, x2):
    """Closure of division (x1/x2) for zero denominator."""
    return torch.where(torch.abs(x2) > 0.001, torch.divide(x1, x2), torch.tensor(1.0, requires_grad=True))
...
add2_torch = _Function(function=torch.add, name="add_torch", arity=2)
sub2 = _Function(function=np.subtract, name="sub", arity=2)
sub2_torch = _Function(function=torch.subtract, name="sub_torch", arity=2)
mul2 = _Function(function=np.multiply, name="mul", arity=2)
mul2_torch = _Function(function=torch.multiply, name="mul_torch", arity=2)
div2 = _Function(function=_protected_division, name="div", arity=2)
div2_torch = _Function(function=_protected_division_torch, name="div_torch", arity=2)
...
_function_map = {
    "add": add2,
    "add_torch": add2_torch,
    "sub": sub2,
    "sub_torch": sub2_torch,
    "mul": mul2,
    "mul_torch": mul2_torch,
    "div": div2,
    "div_torch": div2_torch,
    "sqrt": sqrt1,
    "log": log1,
    "abs": abs1,
    "neg": neg1,
